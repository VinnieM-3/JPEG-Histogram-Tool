...
public class BrightnessPanel extends JPanel implements MouseListener, MouseMotionListener {
	private static final long serialVersionUID = -2029213247598970086L;
	...
	public BrightnessPanel(BufferedImage srcImg) {
	...
	public float calcMagSetting(int pos) {
		return ((float)pos/100f)*0.50f;
	}
	...
	public BufferedImage applyAdj(BufferedImage srcImage, float magSetting, int mode) {
		BufferedImage bufImg = null;
		if ( mode == 1 ) {
			bufImg = ImageAnalysis.BrightnessMode1(srcImage, magSetting, null);
		}else if ( mode == 2 ) {
			bufImg = ImageAnalysis.BrightnessMode2(srcImage, magSetting, null);
		}
		return bufImg;
	}
	...
	protected void paintComponent(Graphics g) {
		...
		if (sliderMoved) {
			...
			dstScaledImage = applyAdj(srcScaledImage, magSetting, getMode());
		}
		...
		if ( (zoomed) ) {
			...
			if (dstScaledImageZoomAdj == null) {
				dstScaledImageZoomAdj = applyAdj(dstScaledImageZoom, magSetting, getMode());
			}
