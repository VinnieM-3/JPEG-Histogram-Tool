...
public class CropPanel extends JPanel implements MouseListener, MouseMotionListener {
	private static final long serialVersionUID = 3409750151682364583L;
	...
	private boolean ratioEnabled = false;
	...
	public CropPanel(BufferedImage srcImg) {
	    ...
	    rectScaledCrop.x = imgX1 + Math.round((float)scaledImageWidth/4.0f);
	    rectScaledCrop.y = imgY1 + Math.round((float)scaledImageHeight/4.0f);
	    rectScaledCrop.width = Math.round((float)scaledImage.getWidth()/2.0f);
	    rectScaledCrop.height = Math.round((float)scaledImage.getHeight()/2.0f);

	    heightWidthRatio = rectScaledCrop.height/rectScaledCrop.width;
	...
	public void enableRatio(boolean setting) {
		ratioEnabled = setting;
		cropRatioEvent = true;
		repaint();
	}
