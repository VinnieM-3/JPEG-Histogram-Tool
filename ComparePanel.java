...
public class ComparePanel extends JPanel implements MouseListener, MouseMotionListener {
	private static final long serialVersionUID = -4686921173525012735L;
	...
	private BufferedImage origDstFullImage = null;
	...
	public ComparePanel(BufferedImage srcImg, BufferedImage dstImg) {
		activeRegions = new ArrayList<ActiveRegion>();
		origFullImage = srcImg;
		origDstFullImage = dstImg;
	    srcScaledImage = ImageAnalysis.getScaledImage(srcImg, imgBoxWidth, imgBoxHeight);
 	    dstScaledImage = ImageAnalysis.getScaledImage(origDstFullImage, imgBoxWidth, imgBoxHeight);    
	...
	protected void paintComponent(Graphics g) {
		...
		g2.drawString("<<< ORIGINAL >>>", cntrLeftBox - 55, yTitlePoint);
		g2.drawString("<<< MODIFIED >>>", cntrRightBox - 55, yTitlePoint);
		...
		int halfBoxHeight = (int)Math.floor((float)imgBoxHeight/2.0f);					
      	...
      	//draw images
		if ( zoomed && (origFullImage.getWidth() == origDstFullImage.getWidth())
				&& (origFullImage.getHeight() == origDstFullImage.getHeight()) ) {
			if ( zoomDrag ){
				...
				dstScaledImageZoom = ImageAnalysis.getImageClip(origDstFullImage, fullPosX, fullPosY, imgWidth, imgHeight);
			}else if (zoomClick) {
				float scaledPosX = 0f;
			...
			if (dstScaledImageZoomAdj == null) {
				dstScaledImageZoomAdj = dstScaledImageZoom;
			}
		...
		LumHistY2 = yImgPoint + imgBoxHeight + 90;
